...
iterate_over_random_files <- function(j, n, rand_index_of_files_all, group1, group2, k) {  #iterations for resampling purpose under a defined participant subset
  ...
  for (i in 1: num_files){
    ...
    lm0= lm(alpha_power ~ 1)
    lm1= lm(alpha_power ~ group)
    sum_lm1= summary(lm1)
    bf=exp(
      0.5*(BIC(lm0)-BIC(lm1)))
...
iterate_over_subject <- function(k, n) {  #iterations for a certain number of participants subjects
  ...
  file=paste('/export02/data/Sapphire_stability/new_results/0.2_alpha_peak_participant_subset_noSubj_', n,'_no_of_iteration_',k, '_relative.csv', sep = '')
...
num_subj= 500
num_feats=751
alpha_row=25:37 #25:37 is for alpha; 40:91 for beta
num_file_iter=1000

load('/export02/data/Sapphire_stability/SimulatedData/alpha_new/0.2_250_PSD/PSD_all_data.rds')

##new version for  relative power##
sum_power=apply(data[,4:121,], c(1,3), sum)
data=apply(data[,alpha_row,],c(1,3),mean)
data= data / sum_power
